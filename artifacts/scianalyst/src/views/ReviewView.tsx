...
export function ReviewView({ agents, apiKeys }: ReviewViewProps) {
  ...
  const nonSpecialAgents = agents.filter((a) => !a.is_reviewer && !a.is_synthesizer);
  const reviewerAgents = agents.filter((a) => a.is_reviewer && !a.is_synthesizer);
  const defaultReviewerId = reviewerAgents.find((a) => a.is_default)?.id ?? reviewerAgents[0]?.id ?? '';
  const [selectedReviewerId, setSelectedReviewerId] = useState<string>('');
  const activeReviewerId = selectedReviewerId || defaultReviewerId;
  const reviewerAgent =
    reviewerAgents.find((a) => a.id === activeReviewerId) ??
    reviewerAgents.find((a) => a.is_default) ??
    reviewerAgents[0];
  ...
  const runReview = async () => {
    ...
    const analyseDocument = async (doc: typeof reviewDocs[0], docIdx: number) => {
      ...
      // No per-document synthesis in Review mode — the chosen Review-integrator
      // (De Reviewer or De Boekenschrijver) brings all per-agent results across
      // all documents together at the end.
    };
    ...
    // Final reviewer step — uses the Review-integrator the user picked.
    if (reviewerAgent) {
          ...
          <div className="flex-1">
            ...
            {reviewerAgents.length > 0 && (
              <div className="mt-3 border-t border-slate-700 pt-3">
                <h2 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">{t('sectionReviewIntegrator')}</h2>
                <div className="space-y-1.5">
                  {reviewerAgents.map((agent) => {
                    const checked = activeReviewerId === agent.id;
                    return (
                      <button
                        key={agent.id}
                        onClick={() => !isRunning && setSelectedReviewerId(agent.id)}
                        disabled={isRunning}
                        className={`w-full flex items-center gap-2.5 p-2.5 rounded-lg border transition-all text-left disabled:cursor-not-allowed ${
                          checked ? 'border-sky-500/50 bg-sky-500/10' : 'border-transparent hover:bg-slate-800/50'
                        }`}
                      >
                        <span className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${checked ? 'border-sky-400' : 'border-slate-600'}`}>
                          {checked && <span className="w-2 h-2 rounded-full bg-sky-400" />}
                        </span>
                        <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: agent.color }} />
                        <span className="text-xs text-slate-300 truncate">{agent.name}</span>
                      </button>
                ...
                <p className="text-xs text-slate-600 mt-2 italic">{t('reviewIntegratorHint')}</p>
              </div>
            )}
          </div>
        )}

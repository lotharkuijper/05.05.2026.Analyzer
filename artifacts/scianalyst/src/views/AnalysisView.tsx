...
export function AnalysisView({ agents, apiKeys }: AnalysisViewProps) {
  ...
  const runAnalysis = async () => {
    ...
    // Synthesis step — Analyse always uses the single is_synthesizer agent
    // (De Eindredacteur). Review-integrators (is_reviewer) are intentionally excluded.
    const synthesisAgent =
      agents.find((a) => a.is_synthesizer && a.is_default && !a.is_reviewer) ??
      agents.find((a) => a.is_synthesizer && !a.is_reviewer);
  ...
  const nonSynthesisAgents = agents.filter((a) => !a.is_reviewer && !a.is_synthesizer);

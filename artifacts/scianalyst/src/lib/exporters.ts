...
// ─── Colour palette ──────────────────────────────────────────────────────────
const C = {
  // Table colours — zeer lichtblauwe achtergrond, zeer donkerblauwe tekst,
  // donkerblauw kader (per gebruikersvoorkeur)
  tableHeaderBg: 'DBEAFE',    // blue-100, lichtblauwe kop
  tableHeaderText: '0C1F3D',  // navy, zeer donkerblauw
  tableBodyText: '0C1F3D',    // navy, zeer donkerblauwe celtekst
  tableRowEven: 'F0F7FF',     // zeer licht blauw
  tableRowOdd: 'E0EEFF',      // iets dieper licht blauw voor zebra
  tableBorderOuter: '1E3A8A', // blue-900, donkerblauw kader
  tableBorderInner: '60A5FA', // blue-400, zichtbare maar zachte celranden
  // Meta-tabel
      ...
      new TableRow({
        children: Array.from({ length: colCount }, (_, ci) =>
          new TableCell({
            children: [
              new Paragraph({
                children: textRuns(cols[ci] ?? '', { size: 20, color: C.tableBodyText }),
                spacing: { before: 80, after: 80 },
              }),
            ],
            shading: rowIdx % 2 === 1
              ? { fill: C.tableRowOdd, type: ShadingType.CLEAR, color: 'auto' }
              : { fill: C.tableRowEven, type: ShadingType.CLEAR, color: 'auto' },
            borders: {
        ...
        new TableRow({
          children: [
            new TableCell({
              ...
              shading: { fill: C.metaLabelBg, type: ShadingType.CLEAR, color: 'auto' },
              // tekst kleur voor labels al via TextRun gezet (donkerblauw)
              borders: {
                top: rowIdx === 0 ? outerBorder : innerBorder,
                bottom: innerBorder,
            ...
            new TableCell({
              children: [
                new Paragraph({
                  children: [new TextRun({ text: value, size: 20, color: C.tableBodyText, font: FONT })],
                  spacing: { before: 80, after: 80 },
                }),
              ],
...
function agentDivider(color: string, name: string, role: string): Paragraph[] {
  ...
  return [
    ...
    new Paragraph({
      ...
      shading: { fill: pastelBg, type: ShadingType.CLEAR, color: 'auto' },
  ...
  if (synthesisResult) {
    children.push(
      ...
      new Paragraph({
        ...
        shading: { fill: C.synthesisBg, type: ShadingType.CLEAR, color: 'auto' },
        spacing: { before: 0, after: 0 },
        border: {

...
const translations = {
  nl: {
    ...
    selectAll: 'Alles',
    synthEditorNote: 'De Eindredacteur wordt automatisch uitgevoerd',
    running: 'Bezig...',
  ...
  en: {
    ...
    selectAll: 'All',
    synthEditorNote: 'The Editor runs automatically',
    running: 'Running...',
...
// Synthesis prompt per language
export function getSynthesisPrompt(lang: Language, documentName: string, combinedResults: string): string {
  if (lang === 'en') {
    return `You are the editor-in-chief of a scientific journal. Combine the analyses below of the paper "${documentName}" into one integrated, clear final report. Follow this structure strictly:

## 1. Overview table

Create a Markdown table with per analyzed dimension a rating on a scale of 1-5 and a one-line summary:
| Dimension | Score (1-5) | Key finding |
|---|---|---|

## 2. Methodological flowchart

Draw an ASCII flowchart visualizing the research method. Use symbols like ->, |, and + for the steps (Research question -> Study design -> Data collection -> Analysis -> Conclusion). Enclose in a code block.

## 3. Integrated findings

Write a numbered section per theme integrating the findings. Use headings (##) per theme.
...
Close with a concise conclusion and at most five numbered recommendations.
...
${combinedResults}`;
  ...
  return `Je bent de eindredacteur van een wetenschappelijk tijdschrift. Combineer de onderstaande analyses van het artikel "${documentName}" tot één geïntegreerd, overzichtelijk eindrapport. Volg deze structuur strikt:
...
export function getReviewerPrompt(lang: Language, articleSummaries: string): string {
  if (lang === 'en') {
    return `You are a systematic review expert. Below are the analyses of multiple scientific articles. Write one integrated academic review document following this structure exactly:

## 1. Overview of included studies
| Study | Design | Population | Main finding | Quality |
|---|---|---|---|---|

## 2. Risk-of-Bias assessment
| Study | Selection | Performance | Detection | Attrition | Reporting | Judgment |
|---|---|---|---|---|---|---|
Use: + (low risk), ? (unclear), - (high risk)

## 3. Synthesis of findings
Per theme (###), describe agreements and contrasts between studies.

## 4. Methodological quality
Summarize methodological strengths and limitations across studies.

## 5. Statistical synthesis
Describe statistical approaches and findings across studies. Discuss heterogeneity where relevant.

## 6. Conclusions and recommendations
State an overarching conclusion and at most five numbered recommendations.
...
${articleSummaries}`;
  ...
  return `Je bent een expert in systematische literatuurreviews. Hieronder staan de analyses van meerdere wetenschappelijke artikelen. Schrijf één geïntegreerd academisch reviewdocument met precies deze structuur:
